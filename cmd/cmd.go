...
func client() {
	...
	for _, server := range CLI.Client.Server {
		if err = m.AddPeer(server, ""); err != nil {
			panic(fmt.Errorf("failed to add server: %w", err))
		}
	}
	...
	var connected bool
	var server string
	var state metalbond.ConnectionState
	for {
		connected = true
		for _, server = range CLI.Client.Server {
	...
	var parts []string
	var routeType pb.NextHopType
	var prefix netip.Prefix
	var dest metalbond.Destination
	var hopIP netip.Addr
	var hop metalbond.NextHop
	var from, to int64
	for _, announcement := range CLI.Client.Announce {
		...
		var ipversion metalbond.IPVersion = metalbond.IPV6
		if prefix.Addr().Is4() {
